...
class DeactivateCustomerHandlerTest {
    ...
    public void ShouldDeactivateCustomer() throws EntityNotFound, Forbidden {
        when(customerRepository.find(any())).thenReturn(
                getCustomer("7680ebe0-fc57-456e-9c60-f8307cf01ad7", CustomerStatus.ACTIVE)
        );
        ...
        DeactivateCustomerHandler handler = new DeactivateCustomerHandler(customerRepository);
        DeactivateCustomerResponse response = handler.execute(command);
    ...
    private DeactivateCustomerResponse getExpected() {
